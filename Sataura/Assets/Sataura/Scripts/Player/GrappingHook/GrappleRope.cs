...
namespace Sataura
{
    public class GrappleRope : MonoBehaviour
    {
        ...
        void DrawRopeNoWaves()
        {
            /*if (tempObject == null)
            {
                tempObject = Instantiate(tempPrefab, grapplingGun.grapplePoint, Quaternion.identity);
                tempObjectRb = tempObject.GetComponent<Rigidbody2D>();
            }

            if (grapplingGun.hookCollided == false)
            {
                Vector2 direction = (Vector2)tempObject.transform.position - (Vector2)this.transform.position;
                tempObjectRb.velocity = -direction.normalized * 1000 * Time.fixedDeltaTime;

            }

            if (Vector2.Distance(tempObject.transform.position, this.transform.position) < .4f)
            {
                Destroy(tempObject);
                this.enabled = false;
                return;
